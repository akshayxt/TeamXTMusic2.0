...
from Akshat.plugins.play import play
from Akshat.plugins.play.pornplay import play
...
@app.on_message(filters.command("mporn"))
...
@app.on_message(filters.command("mxnxx"))
async def get_random_video_info(client, message):
    if len(message.command) == 1:
        await message.reply("Please provide a title to search.")
        return
